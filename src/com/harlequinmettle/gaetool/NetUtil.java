...
public class NetUtil {
	...
	static String collectAndExtractDataFromNetwork(String tk) {
		...
		saveAsText.append(cnnForecast(tk, true).trim());
		saveAsText.append(" ");
		...
		if (fetchFails.get() > 2) {
			DSUtil.addToDatastoreAsText(DSUtil.DEBUG, tk+fetchFails.get(), "data",
					saveAsText.toString());// ///////////////////////////////////////////////////////////////
	...
	static void addToLocalData(String tk, String threadID) {

		String data = "no data";

		switch (TheBackend.threadAction) {
		case 0:
			data = collectAndExtractDataFromNetwork(tk);
			break;
		case 1:// loading data in memcache will not use here
			break;
		case 2:
			data = Big.collectAndExtract_Big_DataFromNetwork(tk);
			break;
		case 3:
			break;
		case 4:
		...
		TheBackend.saveProgress.get(threadID).put(tk, data);

		TheBackend.completeResults.put(tk, data);
	...
	static protected String getHtml(String suf) {
		...
		while (str.length() < 100) {
			...
			try {
				...
				str = ("Must enter a valid URL"+longString);
			} catch (Exception e) {
			...
			if (str.length() < 100) {
				...
				Integer count = errorMap.get(suf);
				if (count == null) {
					errorMap.put(suf, 1);
				} else {
					errorMap.put(suf, count + 1);
				}
				if(false){
				String x = str;
				if (str.length() > 15000)
					x = str.substring(14000, 14500);
				DSUtil.addToDatastoreAsText(DSUtil.DEBUG, suf  ,
						"data", count +"   "+suf + "    ===>" + x);// ///////////////////////////////////////////////////////////////
			}
			}
		...
		//try sleeping between urlfetches in case that is too much per second for quota
		try {
			Thread.sleep(100);
		} catch (InterruptedException e) {

		}
		// if(str.length()<100 || str.contains(s))
		return str;
	}

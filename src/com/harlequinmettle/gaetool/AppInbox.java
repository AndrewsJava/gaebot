...
public class AppInbox extends HttpServlet implements DBLabels {
		...
		try {
			...
			for (int j = 0; j < emailbody.getCount(); j++) {
				...
				if (disposition != null
					...
					//DSUtil.addToDatastoreAsText(DSUtil.DEBUG, nameID, "data"+j  , content);
	...
	if(yORq.equals("nas")){
		...
		Float days = Float.parseFloat(title.replaceAll("\\.txt", "").split(
		"_")[1]);
			...
			for (int j = 0; j < DBSize; j++) {
				...
				// technical data points: last ten days (dayNumber open high low close volume adjClose)
				...
				if (textdata != null) {
					...
					float[] rawData =  validSmallDataSet(textdata,
							sizes);
			 ...
			 DSUtil.saveAsFloatArray2D(DBF, "FUNDAMENTAL_"+ title.replaceAll("\\.txt", ""), data);
			 DSUtil.saveAsFloatArray3D(DBT, "TECHNICAL_"+ title.replaceAll("\\.txt", ""), pdata);
	}
